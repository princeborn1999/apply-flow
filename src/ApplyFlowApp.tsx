...
function Applications({ applications, onAdd, onView, onDelete, onStatus }: { applications:JobApplication[]; onAdd:()=>void; onView:(a:JobApplication)=>void; onDelete:(a:JobApplication)=>void; onStatus:(a:JobApplication,s:ApplicationStatus)=>void }) {
  ...
  return <>
    ...
    <div className="toolbar"><div className="field"><label htmlFor="search">搜尋</label><input id="search" className="input" value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="搜尋公司或職位…"/></div>
      ...
      <div className="field"><label htmlFor="sort">日期排序</label><select id="sort" className="select" value={sort} onChange={(e)=>setSort(e.target.value)}><option value="new">最新優先</option><option value="old">最舊優先</option></select></div>
    </div>
    <section className="panel">{filtered.length===0?<EmptyState/>:<div className="table-wrap"><table><thead><tr><th>Company</th><th>Position</th><th>Applied date</th><th>Status</th><th>Waiting days</th><th>Actions</th></tr></thead><tbody>{filtered.map((a)=><tr key={createApplicationKey(a.company,a.country,a.position)}><td><strong>{a.company}</strong><small>{a.country}</small></td><td>{a.position}</td><td>{a.appliedDate}</td><td><select className="select" aria-label={`更新 ${a.company} 狀態`} value={a.status} onChange={(e)=>void onStatus(a,e.target.value as ApplicationStatus)} style={{ padding: 7, width: 120 }}>{statuses.map((s)=><option key={s}>{s}</option>)}</select></td><td>{waitingDays(a.appliedDate)} days</td><td><div className="actions"><button className="icon-button" aria-label="查看詳細資料" onClick={()=>onView(a)}>↗</button><button className="icon-button" aria-label="刪除紀錄" onClick={()=>onDelete(a)}>×</button></div></td></tr>)}</tbody></table></div>}</section>
  </>;
}
...
function AddApplication({ onCreated, onView }: { onCreated:(a:JobApplication)=>void; onView:(a:JobApplication)=>void }) {
  ...
  return <>
    ...
    <section className="add-card"><div className="add-intro"><div><h2>Paste Job Description</h2><p>Check 階段不會新增任何紀錄。</p></div><StatusBadge status="Waiting"/></div><div className="editor">
      ...
      {parsed&&<div className="parsed-fields"><div className="field"><label htmlFor="parsed-company">公司</label><input id="parsed-company" className="input" value={parsed.company} onChange={(e)=>setParsed({...parsed,company:e.target.value})}/></div><div className="field"><label htmlFor="parsed-country">國家</label><input id="parsed-country" className="input" value={parsed.country} onChange={(e)=>setParsed({...parsed,country:e.target.value})}/></div><div className="field"><label htmlFor="parsed-position">職位</label><input id="parsed-position" className="input" value={parsed.position} onChange={(e)=>setParsed({...parsed,position:e.target.value})}/></div></div>}
