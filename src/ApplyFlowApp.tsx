...
function Applications({ applications, onAdd, onView, onDelete, onStatus }: { applications:JobApplication[]; onAdd:()=>void; onView:(a:JobApplication)=>void; onDelete:(a:JobApplication)=>void; onStatus:(a:JobApplication,s:ApplicationStatus)=>void }) {
  ...
  return <>
    <div className="title-row"><div><h1 className="page-title">申請紀錄</h1><p className="page-subtitle">共 {applications.length} 筆，可依 Company、Country 與 Status 篩選。</p></div><button className="button primary" onClick={onAdd}>＋ Add application</button></div>
