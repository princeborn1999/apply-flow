...
function AddApplication({ onCreated, onView }: { onCreated:(a:JobApplication)=>void; onView:(a:JobApplication)=>void }) {
  const [entryMode,setEntryMode]=useState<"check"|"manual">("manual"); const [jd,setJd]=useState(""); const [parsed,setParsed]=useState<ParsedJob|null>(null); const [analysis,setAnalysis]=useState<JobFitAnalysis|null>(null); const [duplicate,setDuplicate]=useState<JobApplication|null>(null); const [dialog,setDialog]=useState(false); const [quickCopyOpen,setQuickCopyOpen]=useState(false); const [busy,setBusy]=useState(false); const [error,setError]=useState(""); const [manualCompany,setManualCompany]=useState(""); const [manualCountry,setManualCountry]=useState(""); const [manualPosition,setManualPosition]=useState("Frontend Developer");
