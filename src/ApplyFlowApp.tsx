...
import { createApplicationKey, normalizeApplicationText, waitingDays } from "./utils/applicationKey";
    ...
    <main className="main">
      ...
      <div className="content">
        ...
        {loading ? <LoadingState /> : <>
          ...
          {page === "add" && <AddApplication applications={applications} onCreated={addCreated} onView={(app) => go("detail",app)} />}
...
function AddApplication({ applications, onCreated, onView }: { applications:JobApplication[]; onCreated:(a:JobApplication)=>void; onView:(a:JobApplication)=>void }) {
  const [entryMode,setEntryMode]=useState<"check"|"manual">("manual"); const [jd,setJd]=useState(""); const [parsed,setParsed]=useState<ParsedJob|null>(null); const [analysis,setAnalysis]=useState<JobFitAnalysis|null>(null); const [duplicate,setDuplicate]=useState<JobApplication|null>(null); const [dialog,setDialog]=useState(false); const [quickCopyOpen,setQuickCopyOpen]=useState(false); const [busy,setBusy]=useState(false); const [error,setError]=useState(""); const [manualCompany,setManualCompany]=useState(""); const [manualCountry,setManualCountry]=useState(""); const [manualPosition,setManualPosition]=useState("Frontend Developer"); const [manualCompanyMatch,setManualCompanyMatch]=useState<JobApplication|null>(null);
  const isDemoMode = import.meta.env.VITE_APPLYFLOW_DEMO === "true";
  useEffect(()=>{const company=normalizeApplicationText(manualCompany);if(!company){setManualCompanyMatch(null);return;}const timer=window.setTimeout(()=>setManualCompanyMatch(applications.find((application)=>normalizeApplicationText(application.company)===company)??null),350);return()=>window.clearTimeout(timer);},[applications,manualCompany]);
  ...
  const createManual=async()=>{if(!manualCompany.trim()||!manualCountry){setError("Please enter a company and select a country.");return;}setBusy(true);setError("");try{const created=await applicationApi.createApplication({company:manualCompany.trim(),country:manualCountry,position:manualPosition});onCreated(created);setManualCompany("");setManualCountry("");setManualPosition("Frontend Developer");}catch{setError("新增申請紀錄失敗，請稍後再試。");}finally{setBusy(false);}};
  ...
  return <>
    ...
    <section className="add-card"><div className="add-intro"><div><h2>{entryMode === "check" ? "Paste Job Description" : "Manual entry"}</h2><p>{entryMode === "check" ? "Checking will not add a new application." : "Add an application directly without parsing or duplicate checks."}</p></div><div className="add-intro-actions">{!isDemoMode && <button className="button" type="button" onClick={()=>setQuickCopyOpen(true)}>Quick copy</button>}<StatusBadge status="Waiting"/></div></div><div className="editor">
      <div className="entry-mode-tabs" role="tablist" aria-label="Application entry method"><button type="button" role="tab" aria-selected={entryMode === "check"} onClick={()=>{setEntryMode("check");setError("");}}>Check job description</button><button type="button" role="tab" aria-selected={entryMode === "manual"} onClick={()=>{setEntryMode("manual");setError("");}}>Manual entry</button></div>
      {entryMode === "check" ? <>
      ...
      </> : <><div className="manual-fields"><div className="field"><label htmlFor="manual-company">Company</label><input id="manual-company" className="input" value={manualCompany} onChange={(event)=>setManualCompany(event.target.value)} onBlur={()=>setManualCompany((company)=>company.trim())} placeholder="Company name" autoFocus/>{manualCompanyMatch&&<small style={{color:"#9a5b18",marginTop:7,lineHeight:1.45}}>Already applied · {manualCompanyMatch.position} · {manualCompanyMatch.country} · {manualCompanyMatch.appliedDate}</small>}</div><div className="field"><label htmlFor="manual-country">Country</label><select id="manual-country" className="select" value={manualCountry} onChange={(event)=>setManualCountry(event.target.value)}><option value="">Select a country</option>{applicationCountries.map((country)=><option key={country} value={country}>{country}</option>)}</select></div><div className="field"><label htmlFor="manual-position">Position</label><select id="manual-position" className="select" value={manualPosition} onChange={(event)=>setManualPosition(event.target.value)}><option>Frontend Developer</option><option>Fullstack Developer</option></select></div></div>{error&&<div style={{marginTop:14}}><ErrorState message={error}/></div>}<div className="editor-footer"><small>Creates the application immediately. No parsing or duplicate check.</small><button className="button primary" disabled={busy||!manualCompany.trim()||!manualCountry} onClick={()=>void createManual()}>{busy?"Adding…":"Add application →"}</button></div></>}
    </div></section>
